    ...
    : QWidget(parent), m_sceneId(scene.id)
{
    ...
    m_statusLabel = new QLabel(scene.isEnabled ? "已启用" : "已禁用");
    m_statusLabel->setStyleSheet(scene.isEnabled ? 
        "color: #28a745; font-weight: bold; border: 1px solid #28a745; border-radius: 4px; padding: 2px 5px;" : 
        "color: #dc3545; font-weight: bold; border: 1px solid #dc3545; border-radius: 4px; padding: 2px 5px;");
